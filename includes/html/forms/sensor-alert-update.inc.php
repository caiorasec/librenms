...
if (isset($_POST['sub_type']) && ! empty($_POST['sub_type'])) {
    ...
    if (! is_numeric($_POST['device_id']) || ! is_numeric($_POST['sensor_id'])) {
        ...
        if (dbUpdate(['sensor_alert' => $state], 'sensors', '`sensor_id` = ? AND `device_id` = ?', [$_POST['sensor_id'], $_POST['device_id']]) >= 0) {
            ...
            $message = 'Alerts ' . $state_string . ' for sensor ' . htmlspecialchars((string) $_POST['sensor_desc']);
        } else {
            $status = 'error';
            $message = 'Couldn\'t ' . substr($state_string, 0, -1) . ' alerts for sensor ' . htmlspecialchars((string) $_POST['sensor_desc']) . '. Enable debug and check librenms.log';
        }
    }
}
$response = [
